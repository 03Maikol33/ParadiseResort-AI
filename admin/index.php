...
try {
    ...
    $stmtRev = db()->query('SELECT COALESCE(SUM(total_price), 0) as tot FROM bookings WHERE status_id IN (3, 5)');
    $block->setContent('stat_revenue', '€ ' . number_format((float)$stmtRev->fetch()['tot'], 2, ',', '.'));
    ...
    $block->setContent('stat_revenue', '€ 0,00');
